...
public class PayByCardMessageConverter extends ZP2BankCoreMessageConverter {
	...
	protected void convertAdditionalRequestInfos(InvokePaymentBankConnectorRequest source, DefaultRequest data,
		...
		try {
			...
			reqData.holderName = Strings
					.nullToEmpty((String) source.dataObj.paymentInfo.get(ApiParamsConst.PAYMENTINFO_HOLDERNAME));
			reqData.cvv = Strings.nullToEmpty((String) source.dataObj.paymentInfo.get(ApiParamsConst.PAYMENTINFO_CVV));
			reqData.issueDate = Strings
					.nullToEmpty((String) source.dataObj.paymentInfo.get(ApiParamsConst.PAYMENTINFO_ISSUEDDATE));
			reqData.expireDate = Strings
					.nullToEmpty((String) source.dataObj.paymentInfo.get(ApiParamsConst.PAYMENTINFO_EXPIREDDATE));
